...
xmlrpclib = function()
    ...
    function obj:parseParam(param)
        ...
        elseif element.struct then
            ...
            if element.struct.member then
                for i=1,getn(element.struct.member) do
        ...
        elseif element.array then
            ...
            if element.array.data.value then
                if getn(element.array.data.value) == 0 then
                    values = {element.array.data.value}
                else
                    values = element.array.data.value
                end
                for i=1,getn(values) do
                    tinsert(array,self:parseParam({value = values[i]}))
                end
                array.n = nil
            end
            return array
    ...
    function obj:parseMethodCall(root)
        ...
        if methodCall.params then
            for i=1,getn(methodCall.params.param) do 
                tinsert(params,self:parseParam(methodCall.params.param[i]))
            end
        end
        params.n = nil
